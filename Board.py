...
class Board:
    ...
    def decode_board(self):
        ...
        for i in range(10):
            for j in range(9):
                if self.situation[i][j]!=0:
                    net_input[self.situation[i][j]//100+7][i][j]=1
                    if self.situation[i][j]>0:
                        net_input[15][i][j] = 1
                    else:
                        net_input[16][i][j] = 1
                net_input[7][i][j]=abs(self.current_player_start)
        a = self.next_move//100
        net_input[17][9-a//10][8-a%10] = 1
        ...
        for i in self.valid_move:
            ...
            if i < 90:
                net_input[18][i // 9][i% 9] = 1
            elif i <180:
                net_input[19][(i-90) // 9][(i-90) % 9] = 1
            else:
                net_input[20][(i - 180) // 9][4] = 1
        return net_input
